...
def test_refresh_fails_when_running_runtime_does_not_restart(tmp_path, monkeypatch):
    vault = tmp_path / "vault"
    vault.mkdir()
    (vault / "note.md").write_text("# note\n", encoding="utf-8")
    state = tmp_path / "state"
    ...
    runtime_state.mkdir()
    statuses = iter([
        {"ok": True, "endpoint_healthy": True, "pid_alive": True, "pid_owned": True},
        {"ok": False, "endpoint_healthy": True, "pid_alive": False, "pid_owned": False},
    ])

    monkeypatch.setattr(refresh.index, "qmd_available", lambda: True)
    monkeypatch.setattr(refresh.index, "_QMD", "/tmp/qmd")
    monkeypatch.setattr(refresh.index, "_collection_exists", lambda _name: True)
    monkeypatch.setattr(refresh.qmd_runtime, "status", lambda: next(statuses))
    monkeypatch.setattr(refresh.qmd_runtime, "stop", lambda: {"stopped": True})
    monkeypatch.setattr(refresh.qmd_runtime, "start", lambda _binary: {"started": False})
    monkeypatch.setattr(refresh.qmd_runtime, "doctor", lambda *args: {
        "collection_present": True, "pending_embeddings": 1,
    })
    monkeypatch.setattr(refresh.qmd_runtime, "ensure_dirs", lambda: {
        "maintenance": runtime_state / "maintenance.json",
    })
    ...
    assert report["ok"] is False
    assert report["outcome"] == "failed"
    assert "did not restart" in report["error"]
