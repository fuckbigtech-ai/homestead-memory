...
# 2. reader: set HSM_READER (prompt on stdin) or have ~/.local/bin/cc (GLM) present
export HSM_READER="ollama run llama3.1:8b"      # example; any local/flat-rate reader

# 3. run (start small, then scale to the full 500)
python -m homestead_memory.benchmarks.longmemeval --data <path-to>/longmemeval_s.json -n 20 --mode both
python -m homestead_memory.benchmarks.longmemeval --data <path-to>/longmemeval_s.json --mode both
```
